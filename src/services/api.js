...
class VinFastAPI {
    ...
    async fetchLocationName(lat, lon) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const strip = (s) => s ? s.replace(/^(Thành phố|Tỉnh|Quận|Huyện|Xã|Phường)\s+/gi, "").trim() : s;

                const rawDistrict = a.city_district || a.district || a.county;
                const rawCity = a.city || a.town || a.village || a.state || a.province;

                return {
                    location_address: [strip(rawDistrict), strip(rawCity), (a.country_code || "VN").toUpperCase()].filter(Boolean).join(", "),
                    weather_address: [strip(rawCity), (a.country_code || "VN").toUpperCase()].filter(Boolean).join(", "),
                };
            }
        } catch (e) {
    ...
    async getTelemetry(vin, deepScan = false) {
        if (vin) {
            this.vin = vin;
            this.saveSession();
        }
