...
2.  **Refresh Logic**:
    ...
    *   *(Current Buffer)*: It currently logs the user out to force re-authentication (Simplest secure path). 
    *   *Future*: Implement `/api/refresh` proxy to exchange `refresh_token` for new `access_token`.
